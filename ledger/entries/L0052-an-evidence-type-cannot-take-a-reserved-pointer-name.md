...
- 2026-09-11T03:10:01-07:00 · corroborated · grade: measured · author: main
  ...
  note: read against the working tree after from_table gained, since the reading at 2a76453, a check that `citation-placement` is one of its three outcomes and passes it into Config: the intersection of the declared evidence types with the reserved pointer names and the error listing them are unchanged; the assertion holds as written.
...
- 2026-09-14T19:22:21-07:00 · corroborated · grade: measured · author: main
  ...
  note: re-read after the commit that adds the `merge-renumber` key. The reserved-name refusal is untouched: an evidence type named `entry`, `source`, `search` or `defect` is still refused where it always was.
...
- 2026-09-14T20:21:11-07:00 · corroborated · grade: measured · author: main
  ...
  note: re-read after the commit that fixes what the pre-merge gate found. The section lost a verbatim duplicate of the merge-renumber check, which had been written into it twice; the reserved-name refusal is untouched.
...
- 2026-09-20T12:56:58-07:00 · corroborated · grade: measured · author: main
  ...
  note: re-read after the commit that adds the `citation-slug` key. The section gained one more check, written like the two beside it and placed after them. The reserved-name check is untouched and sits where it did.
