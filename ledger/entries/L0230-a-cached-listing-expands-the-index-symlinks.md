...
- 2026-09-12T16:00:57-07:00 · corroborated · grade: measured · author: main
  evidence: code: src/claims_ledger/schema.py § "index_tree" =sha256:2f7167cc782fc0bd1a13a823b51e66998c53f568bec5d3743e30ea041a0564b6
  note: the Assertion stands and the bound is answered differently, which corrects a regression the bound itself introduced (the fix-review gate on this branch (qe ticket 7b317b5ea95e4670)). Reaching it returned a `why`, and a `why` sends the WHOLE cached listing to the working tree — so `ln -s . here`, an ordinary self-referential link nested none deep, reached one segment further every pass, tripped the depth branch, and took a staged-only document off the list entirely: measured, `references --cached` clean at exit 0 where the commit before it exited 1 naming the document. A truncation is not a failure and a narrower universe than the commit has is not a wrong one, so the listing keeps what it reached and the guard says what it did not. The old wording was false besides — "nested more than 16 deep" of a link nested none.
