...
- 2026-09-20T12:56:58-07:00 · corroborated · grade: measured · author: main
  ...
  note: re-read after the commit that lets a marker name its entry by the series and number alone. The document loop in this section now resolves each marker through `cited_target` and collects the resolved id rather than the text of the marker, and the slug rule is asked after the lookup. The act is still checked against the target's current status, on both the entry-to-entry loop and the document loop; what changed is which entry the document loop looks the status up for, and it is now the one the marker resolves to rather than the one whose id it spells.
